...
private struct TemplateRow: View {
    ...
    /// "6 items" for an unowned bag, "6 items · Jeff" once it's been
    /// assigned to a traveler — otherwise there's no way to tell an owned
    /// bag apart from a general-purpose one without opening it.
    private var subtitle: String {
        let itemCount = "\(template.items.count) item\(template.items.count == 1 ? "" : "s")"
        guard let owner = template.owner else { return itemCount }
        return "\(itemCount) · \(owner.name)"
    }
}
